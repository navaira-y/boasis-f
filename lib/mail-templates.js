...
/* the line the reviewed copy ends each visitor email with */
const TAGLINE = 'The most advanced AI for company formation and management in the UAE.';
...
function shell({ preheader, title, intro, body, footer, legal = true }) {
  return `<!doctype html>
...
<p style="margin:0;font-size:12px;line-height:1.7;color:${BRAND.muted}">${footer || ''}</p>
${legal ? `<p style="margin:6px 0 0;font-size:11px;line-height:1.7;color:#8A99AA">Boasis &middot; FZC &middot; United Arab Emirates &middot; <a href="${BRAND.site}" style="color:${BRAND.blue};text-decoration:none">boasis.ae</a></p>` : ''}
</td></tr>
...
function contactToUser({ name, email, message, at }) {
  ...
  const footer = `${TAGLINE}<br>This is an automatic acknowledgement of the message sent from boasis.ae with ${esc(email)}. BOASIS, United Arab Emirates.`;
  return {
    ...
    text: [
      ...
      'While you wait',
      'Book a demonstration: thirty minutes, live, with the team.',
      `${BRAND.site}/contact.html`,
      'Join the early access list: be among the first inside the platform.',
      `${BRAND.site}/#manage`, '',
      ...
      TAGLINE,
      `This is an automatic acknowledgement of the message sent from boasis.ae with ${email}. BOASIS, United Arab Emirates.`,
    ].join('\n'),
  };
}
...
/* ───────── 6 · the demo booking → to the visitor ────────────────────────────
   The reviewed copy. Three fields it cannot carry, because they do not exist on our
   side: the date, the time and the joining link are held by Google's booking page, not
   by this server. Google mails them the moment the visitor books, so the line below
   says where they are instead of promising something this mail does not contain. */
function demoToUser({ name, email, entity }) {
  ...
  const intro = "Thank you for taking the time to book a session with us. Your demonstration of the BOASIS platform is confirmed, and everything you need to join is in Google's confirmation, sent to this address the moment you chose a time.";
  const footer = `${TAGLINE}<br>You are receiving this message because a demonstration was booked with ${esc(email)} on boasis.ae. BOASIS, United Arab Emirates.`;
  return {
    subject: 'Demo confirmed · BOASIS',
    preheader: 'Your thirty minutes with us, the joining link, and what we will cover.',
    html: shell({
      ...
      body: [
        ...
        para(`Your joining link, the date and time you chose, the dial in number and the one click reschedule or cancel are all in that confirmation. If it has not arrived, check the spam folder, or write to <a href="mailto:${BRAND.mail}" style="color:${BRAND.blue};text-decoration:none">${BRAND.mail}</a> and we will send it ourselves.`),
    ...
    text: [
      ...
      `Your joining link, the date and time you chose, the dial in number and the one click reschedule or cancel are all in Google's confirmation. If it has not arrived, check the spam folder, or write to ${BRAND.mail} and we will send it ourselves.`, '',
      'What we will cover', '',
      ...
      TAGLINE,
      `You are receiving this message because a demonstration was booked with ${email} on boasis.ae. BOASIS, United Arab Emirates.`,
...
/* ───────── 8 · early access → to the visitor ───────────────────────────────
   The reviewed copy, whole. The unsubscribe link the review shows is the last thing
   we do not have a route for, so the line names the address a person actually reads. */
function manageToUser({ name, email, at }) {
  ...
  const footer = `${TAGLINE}<br>You joined the early access list with ${esc(email)} on ${esc(joined)}. We will only write to you about access, and nothing else. <a href="mailto:${BRAND.mail}?subject=Remove%20me%20from%20the%20early%20access%20list" style="color:${BRAND.blue};text-decoration:none">Remove me from the list</a>. BOASIS, United Arab Emirates.`;
  const body = [
  ...
  return {
    ...
    html: shell({ preheader: `Your place is reserved, ${first}.`, title, intro, body, legal: false, footer }),
    text: [
      title, '',
      intro, '',
      ...
      'The BOASIS Team', BRAND.mail, BRAND.site, '',
      TAGLINE,
      `You joined the early access list with ${email} on ${joined}. We will only write to you about access, and nothing else. Remove me from the list: write to ${BRAND.mail}. BOASIS, United Arab Emirates.`,
    ].join('\n'),
  };
}
