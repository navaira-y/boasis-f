...
function manageToOwner({ name, email, phone, have, count, authority, at }) {
  ...
  return {
    ...
    html: shell({
      ...
      body: panel([
        row('Name', name), row('Email', email), row('Phone', phone),
        row('Company', haveLabel), row('How many', countLabel),
        authority ? row('Authority', authority) : '', row('When', dateLong(at)),
      ].join('')),
      footer: `Reply to this email to answer ${esc(email)} directly.`,
    }),
    text: [
      ...
      haveLabel ? row('Company', haveLabel) : '',
      countLabel ? row('How many', countLabel) : '',
