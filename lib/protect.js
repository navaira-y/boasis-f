...
function globalGate({ name, perMinute, perDay, onTrip }) {
  /* Rolling windows, counted from the first request rather than from the clock. A window
     tied to the wall clock doubles its allowance at the boundary — a flood that starts at
     12:59:59 gets the full minute twice — and it also made the suite flaky for the same
     reason. This counts a real minute and a real day. */
  const MIN = 60 * 1000, DAY = 24 * 60 * MIN;
  let minuteFrom = 0, minuteCount = 0, minuteTold = false;
  let dayFrom = 0, dayCount = 0, dayTold = false;
  return (req, res, next) => {
    const now = Date.now();
    if (now - minuteFrom >= MIN) { minuteFrom = now; minuteCount = 0; minuteTold = false; }
    if (now - dayFrom >= DAY) { dayFrom = now; dayCount = 0; dayTold = false; }
    minuteCount += 1;
    dayCount += 1;
    ...
    if (over === 'minute' ? !minuteTold : !dayTold) {
      ...
      const limit = over === 'minute' ? perMinute + ' a minute' : perDay + ' today';
      console.log(`[limit] ${name}: the whole site passed ${limit} · refusing more`);
      /* the owner hears about a flood; the callback is the server's business, and a fault
         in it must never break the refusal itself */
      if (typeof onTrip === 'function') { try { onTrip({ name, over, limit }); } catch (e) {} }
    }
...
let trapLines = 0;
let trapHidden = 0;
function noteTrap(why, where, verb = 'dropped') {
  ...
  trapLines++;
  console.log(`[traps] ${verb} · ${why} · ${where}`);
}
