...
function inspect(raw) {
  ...
  if (clean === '/api/first-visit' || clean === '/api/waitlist' || clean === '/api/contact' || clean === '/api/health') return { ok: true, why: 'api' };
...
function securityHeaders(req, res, next) {
  ...
  /* A real CSP, not 'default-src self': the fonts come from googleapis/gstatic and would
     be blocked. report-only would let the site break quietly in production instead, so this
     is enforced. `unsafe-inline` for styles is unavoidable here: the design writes inline
     style attributes, and every email/section reveal depends on them. */
